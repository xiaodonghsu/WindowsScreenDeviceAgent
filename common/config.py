...
def load_env():
    _ = load_dotenv()
    import os
    if not os.getenv("CMS_BASEURL") is None:
        CMS_BASEURL = os.getenv("CMS_BASEURL")
    if  not os.getenv("IOT_DEVICE_NAME") is None:
        IOT_DEVICE_NAME = os.getenv("IOT_DEVICE_NAME")
    return CMS_BASEURL, IOT_DEVICE_NAME
...
def download_config(cms_baseurl:str="", device_name:str="", cms_token:str="", config_path:str=CONFIG_FILE):
    ...
    with open(config_path, "w", encoding="utf-8") as f:
        json.dump(response_json.get("data")[0].get("config", {}), f, ensure_ascii=False)

    return response_json.get("data")[0].get("config", {})
